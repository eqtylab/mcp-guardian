...
const PropertyPanel: React.FC<PropertyPanelProps> = ({ node, onChange, disabled = false }) => {
        ...
        <div className={`pb-4 relative overflow-hidden mb-6`}>
          ...
          <div className="relative">
            ...
            <div className="flex items-center gap-2 mb-4">
              ...
              <div className={`flex-shrink-0 w-10 h-10 bg-${info.color}-100 dark:bg-${info.color}-900/30 rounded-md flex items-center justify-center`}>
                {renderIcon(info.icon, info.color)}
              </div>
              
              <div>
                <h3 className="font-medium text-lg text-foreground">{info.title}</h3>
                <p className="text-sm text-muted-foreground cyber-text-glow">
                  {info.tagline}
                </p>
              </div>
            </div>
            ...
            <div className="mb-5">
              ...
              <div className={`bg-${info.color}-50/20 dark:bg-${info.color}-900/10 rounded p-4 border border-${info.color}-200/30 dark:border-${info.color}-800/30`}>
                {node.type === 'filter' && (
                  <ul className="space-y-2 text-sm pl-4 list-disc">
                    <li>Use <strong>Match Action</strong> to control what happens when the filter condition is met</li>
                    <li>Use <strong>Non-Match Action</strong> for the opposite case</li>
                ...
                {node.type === 'messagelog' && (
                  <ul className="space-y-2 text-sm pl-4 list-disc">
                    <li>Use <strong>Error</strong> level only for critical issues that need attention</li>
                    <li>Use <strong>Info</strong> level for general activity tracking</li>
                    <li>Use <strong>Debug</strong> or <strong>Trace</strong> for detailed troubleshooting</li>
            ...
            <div className="bg-primary/10 rounded-lg p-4 border border-primary/30 mb-5">
              ...
              <p className="text-sm text-muted-foreground">
                Configure this interceptor using the form fields below. Select different nodes in the diagram to edit their respective settings.
              </p>
              <p className="text-xs text-muted-foreground mt-2 pt-2 border-t border-primary/20">
                <strong>NEW:</strong> Node configuration is now available directly in the nodes. Click the dropdown button in each node to access configuration options.
              </p>
            </div>
  ...
  // Function to get the filter type from filter logic
  const getFilterType = (filterLogic: any): string => {
    if (!filterLogic || typeof filterLogic !== 'object') return 'direction';
    
    if ('direction' in filterLogic) return 'direction';
    if ('message_type' in filterLogic) return 'message_type';
    if ('request_method' in filterLogic) return 'request_method';
    if ('and' in filterLogic) return 'and';
    if ('or' in filterLogic) return 'or';
    if ('not' in filterLogic) return 'not';
  ...
  const renderFilterForm = () => {
    ...
    const filterType = getFilterType(filterLogic);
    const info = getInterceptorInfo();
    ...
    return (
      <>
        ...
        <div className="border-t border-border -mx-4 px-4 py-4 bg-muted/30">
          ...
          <p className="text-sm text-muted-foreground mb-3">
            This interceptor can be configured directly in the node. Click the dropdown button in the node to expand it and access configuration options.
          </p>
