...
export interface GuardProfileManualApprovalNodeData extends GuardProfileNodeData {
  type: "ManualApproval";
  log_level: string;
}

export interface GuardProfileManualApprovalNode extends Node {
  type: "ManualApproval";
  data: GuardProfileManualApprovalNodeData;
}

export interface GuardProfileManualApprovalNodeProps extends NodeProps {
  data: GuardProfileManualApprovalNodeData;
}

// Use NodeProps from ReactFlow - fixed for proper type compatibility
const ManualApprovalNode = ({ selected }: GuardProfileManualApprovalNodeProps) => {
  return (
    <div
      className={`node approval-node rounded-lg border p-3 bg-background shadow-sm max-w-[180px] ${
    ...
    >
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="text-xs text-muted-foreground truncate">User confirmation</div>
        </div>
      </div>
